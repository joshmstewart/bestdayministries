...
function styleStandardTablesOnly(html: string): string {
  // Only touch tables inside the *campaign body* (not header/footer) and skip magazine two-column tables.
  return (html || "").replace(
...
serve(async (req) => {
  ...
  try {
    ...
    htmlContent += styleEmptyParagraphs(styleStyledBoxes(styleStandardTablesOnly(campaign.html_content)));
