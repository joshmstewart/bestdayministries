...
serve(async (req) => {
  ...
  try {
    ...
    // Add campaign content (apply email-safe formatting to column layouts, magazine layouts, and standard tables)
    htmlContent += styleMagazineLayouts(styleColumnLayoutTables(styleStandardTablesOnly(campaign.html_content)));
