...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Total words used so far: ${usedWordsSet.size}`);
    ...
    console.log(`Available words from curated list: ${availableWords.length}`);

    // Use AI to pick the best word for today's theme from the curated list
    if (!lovableApiKey) {
    ...
    if (!word || !availableWords.includes(word)) {
      ...
      const { data: newWord, error: insertError } = await supabase
        ...
        .insert({
          ...
          word_date: today,
          hint: `Related to ${randomTheme.name}`
        })
        .select()
        .single();
      ...
      return new Response(
        JSON.stringify({ success: true, message: "New word generated (fallback)", wordId: newWord.id, theme: randomTheme.name }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
