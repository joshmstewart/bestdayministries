-- Fix the sponsor_bestie_funding_progress view with proper JSONB text extraction
  ...
  COALESCE(SUM(
    CASE 
      WHEN s.frequency = 'monthly' 
        AND s.status = 'active' 
        AND s.stripe_mode = (SELECT setting_value#>>'{}' FROM app_settings WHERE setting_key = 'stripe_mode')
      THEN s.amount 
      ELSE 0 
    END
