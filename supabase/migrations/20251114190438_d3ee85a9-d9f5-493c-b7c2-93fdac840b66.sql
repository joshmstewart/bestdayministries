-- Function to generate missing receipts for donations
-- This creates receipt records for donations that exist but have no receipt
CREATE OR REPLACE FUNCTION generate_missing_receipts()
RETURNS TABLE(
  donation_id uuid,
    ...
    BEGIN
      ...
      INSERT INTO sponsorship_receipts (
        ...
        COALESCE(
          ...
          donation_record.stripe_payment_intent_id,
          donation_record.stripe_subscription_id,
          'manual_' || donation_record.id::text
        ),
...
-- View to analyze donations missing receipts
CREATE OR REPLACE VIEW donations_missing_receipts AS
SELECT 
  d.id as donation_id,
  d.donor_email,
  d.amount,
