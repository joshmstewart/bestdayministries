-- Recreate the community_feed_items view with correct column names
CREATE OR REPLACE VIEW public.community_feed_items WITH (security_invoker = true) AS
-- Beats
...
-- Discussion Posts - filters by share_to_feed and marks fortune posts with different item_type
SELECT dp.id,
    CASE 
...
-- Drinks (using generated_image_url instead of image_url)
SELECT cd.id,
...
SELECT ca.id,
    ...
    NULL::bigint AS comments_count,
    jsonb_build_object(
        'link_url', ca.link_url,
        'announcement_type', ca.announcement_type,
        'price_coins', ca.price_coins,
        'is_free', ca.is_free
    ) AS extra_data,
    NULL::uuid AS repost_id
