-- Fix the sponsor_bestie_funding_progress view to properly extract stripe_mode
  ...
  COALESCE(SUM(
    CASE 
      WHEN s.frequency = 'monthly' 
        AND s.status = 'active' 
        AND s.stripe_mode = TRIM(BOTH '"' FROM (SELECT setting_value::text FROM app_settings WHERE setting_key = 'stripe_mode'))
      THEN s.amount 
      ELSE 0 
    END
