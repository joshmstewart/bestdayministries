-- Drop and recreate the view to include extra_data for beats
DROP VIEW IF EXISTS public.community_feed_items;

CREATE VIEW public.community_feed_items AS
-- Beat creations
SELECT 
  id,
  'beat'::text AS item_type,
  name AS title,
  NULL::text AS description,
  creator_id AS author_id,
  created_at,
  image_url,
  likes_count,
  NULL::bigint AS comments_count,
  jsonb_build_object('pattern', pattern, 'tempo', tempo) AS extra_data
FROM public.beat_pad_creations
WHERE is_public = true

UNION ALL

-- User colorings (join with coloring_pages to get title)
SELECT 
  uc.id,
  'coloring'::text AS item_type,
  COALESCE(cp.title, 'Coloring') AS title,
  NULL::text AS description,
  uc.user_id AS author_id,
  uc.created_at,
  uc.thumbnail_url AS image_url,
  uc.likes_count,
  NULL::bigint AS comments_count,
  NULL::jsonb AS extra_data
FROM public.user_colorings uc
LEFT JOIN public.coloring_pages cp ON uc.coloring_page_id = cp.id
WHERE uc.is_public = true

UNION ALL

-- User cards
SELECT 
  id,
  'card'::text AS item_type,
  COALESCE(title, 'Card') AS title,
  NULL::text AS description,
  user_id AS author_id,
  created_at,
  thumbnail_url AS image_url,
  likes_count,
  NULL::bigint AS comments_count,
  NULL::jsonb AS extra_data
FROM public.user_cards
WHERE is_public = true

UNION ALL

-- Discussion posts
SELECT 
  dp.id,
  'post'::text AS item_type,
  dp.title,
  dp.content AS description,
  dp.author_id,
  dp.created_at,
  dp.image_url,
  0 AS likes_count,
  (SELECT COUNT(*) FROM discussion_comments WHERE post_id = dp.id)::bigint AS comments_count,
  NULL::jsonb AS extra_data
FROM public.discussion_posts dp
WHERE dp.is_moderated = true

UNION ALL

-- Albums
SELECT 
  a.id,
  'album'::text AS item_type,
  a.title,
  a.description,
  a.created_by AS author_id,
  a.created_at,
  a.cover_image_url AS image_url,
  0 AS likes_count,
  (SELECT COUNT(*) FROM album_images WHERE album_id = a.id)::bigint AS comments_count,
  NULL::jsonb AS extra_data
FROM public.albums a
WHERE a.is_active = true AND a.is_public = true

UNION ALL

-- Chore challenge gallery
SELECT 
  id,
  'chore_art'::text AS item_type,
  COALESCE(title, 'Chore Challenge Art') AS title,
  NULL::text AS description,
  user_id AS author_id,
  created_at,
  image_url,
  likes_count,
  NULL::bigint AS comments_count,
  NULL::jsonb AS extra_data
FROM public.chore_challenge_gallery

UNION ALL

-- Events
SELECT 
  id,
  'event'::text AS item_type,
  title,
  description,
  created_by AS author_id,
  created_at,
  image_url,
  0 AS likes_count,
  NULL::bigint AS comments_count,
  NULL::jsonb AS extra_data
FROM public.events
WHERE is_active = true AND is_public = true

UNION ALL

-- Prayer requests
SELECT 
  id,
  'prayer'::text AS item_type,
  title,
  content AS description,
  user_id AS author_id,
  created_at,
  image_url,
  likes_count,
  NULL::bigint AS comments_count,
  NULL::jsonb AS extra_data
FROM public.prayer_requests
WHERE is_public = true

UNION ALL

-- Workout generated images
SELECT 
  id,
  'workout'::text AS item_type,
  COALESCE(activity_name, 'Workout Photo') AS title,
  NULL::text AS description,
  user_id AS author_id,
  created_at,
  image_url,
  likes_count,
  NULL::bigint AS comments_count,
  NULL::jsonb AS extra_data
FROM public.workout_generated_images
WHERE is_shared_to_community = true

UNION ALL

-- Public recipes
SELECT 
  id,
  'recipe'::text AS item_type,
  title,
  description,
  creator_id AS author_id,
  created_at,
  image_url,
  likes_count,
  NULL::bigint AS comments_count,
  jsonb_build_object('ingredients', ingredients, 'steps', steps) AS extra_data
FROM public.public_recipes
WHERE is_active = true

UNION ALL

-- Custom drinks
SELECT 
  id,
  'drink'::text AS item_type,
  name AS title,
  description,
  creator_id AS author_id,
  created_at,
  generated_image_url AS image_url,
  likes_count,
  NULL::bigint AS comments_count,
  NULL::jsonb AS extra_data
FROM public.custom_drinks
WHERE is_public = true

UNION ALL

-- Saved jokes
SELECT 
  id,
  'joke'::text AS item_type,
  category AS title,
  answer AS description,
  user_id AS author_id,
  created_at,
  NULL::text AS image_url,
  likes_count,
  NULL::bigint AS comments_count,
  jsonb_build_object('question', question, 'answer', answer) AS extra_data
FROM public.saved_jokes
WHERE is_public = true

ORDER BY created_at DESC;