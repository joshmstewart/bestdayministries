-- Step 1: Link existing unlinked receipts to their donations
-- This updates receipts that have Stripe payment intent IDs but match donations by email/amount/time

UPDATE sponsorship_receipts sr
SET transaction_id = 'donation_' || d.id::text
FROM donations d
WHERE sr.transaction_id NOT LIKE 'donation_%'
  AND sr.sponsorship_id IS NULL  -- Only update receipts not linked to sponsorships
  AND sr.sponsor_email = d.donor_email
  AND sr.amount = d.amount
  AND sr.stripe_mode = d.stripe_mode
  AND ABS(EXTRACT(EPOCH FROM (sr.created_at - d.created_at))) < 86400
  AND NOT EXISTS (
    -- Don't update if donation already has a properly linked receipt
    SELECT 1 FROM sponsorship_receipts sr2
    WHERE sr2.transaction_id = 'donation_' || d.id::text
  );

-- Step 2: Fix generate_missing_receipts function to only check transaction_id
-- Removes fuzzy matching OR clause that was causing receipts to be skipped incorrectly
...
BEGIN
  ...
  -- Find all donations without receipts - ONLY check transaction_id for clean linkage
