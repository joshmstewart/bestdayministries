...
BEGIN
  ...
  IF NOT (public.has_role(_caller, 'caregiver'::public.app_role) OR public.has_admin_access(_caller)) THEN
  ...
  WHERE p.friend_code = _friend_code
    AND EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = p.id AND ur.role = 'bestie'::public.app_role)
  LIMIT 1;
