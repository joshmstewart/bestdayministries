-- Fix announcement timestamp bug: use COALESCE to fall back to created_at when published_at is NULL
-- This prevents "56 years ago" display when published_at is NULL (which JavaScript treats as Unix epoch)
...
SELECT wi.id,
    ...
    NULL::bigint AS comments_count,
    jsonb_build_object('location_name', wi.location_name, 'avatar_id', wi.avatar_id) AS extra_data,
    NULL::uuid AS repost_id
...
-- Content Announcements (FIXED: use COALESCE to handle NULL published_at)
SELECT ca.id,
