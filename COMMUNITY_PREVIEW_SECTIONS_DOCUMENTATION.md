...
3. **Filter:** Only posts with `approval_status = "approved"`
4. **Sort:** Most recent first (`created_at DESC`)
5. **Limit:** 1 post only
6. **Result:** Uses `maybeSingle()` - returns null if no posts exist

**CRITICAL:** The query does NOT filter by `is_moderated` - only by `approval_status`. This ensures discussions load correctly.
...
### 1. Discussion Query
- **MUST NOT** filter by `is_moderated` field
- **MUST** filter only by `approval_status = 'approved'`
