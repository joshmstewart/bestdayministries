...
import { SceneBuilder } from "@/components/chores/SceneBuilder";
...
export function MonthlyChallengeCard({ userId }: MonthlyChallengeCardProps) {
  ...
  const { theme, progress, loading, unplacedStickerCount } = useMonthlyChallenge(userId || '');
  ...
  return (
    <>
      <Card className="relative overflow-hidden border-2 border-primary/20 bg-gradient-to-br from-background to-primary/5 mb-6">
        {/* Decorative corner */}
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="w-20 h-20 rounded-lg bg-muted border-2 border-dashed border-primary/30 flex items-center justify-center overflow-hidden">
                {progress?.placed_stickers && Array.isArray(progress.placed_stickers) && progress.placed_stickers.length > 0 ? (
              ...
              <Button size="sm" onClick={() => setBuilderOpen(true)} className="gap-1">
