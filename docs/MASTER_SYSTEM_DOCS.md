...
## 🚨🚨🚨 ERROR_TOAST_STANDARD - ABSOLUTELY MANDATORY 🚨🚨🚨

**⛔ THIS IS NON-NEGOTIABLE. ZERO EXCEPTIONS. EVER. ⛔**

### THE IRON LAW OF ERROR TOASTS:
ALL error messages in this application MUST be:
1. **RED** - Use variant: "destructive" 
2. **PERSISTENT** - Use duration: Infinity (NEVER auto-dismiss)
3. **COPYABLE** - Include a copy button for debugging

### THE ONLY ACCEPTABLE WAY TO SHOW ERRORS:
```tsx
...
// In your catch block:
  ...
  showErrorToastWithCopy("Doing X", error);
}
```

### ⛔ FORBIDDEN PATTERNS - NEVER USE THESE ⛔
```tsx
// ❌ FORBIDDEN - toast.error() from sonner
toast.error("Failed to save");

// ❌ FORBIDDEN - toast with destructive variant without copy
toast({ title: "Error", description: error.message, variant: "destructive" });

// ❌ FORBIDDEN - inline error messages without persistence
toast.error(`Failed: ${error.message}`);

// ❌ FORBIDDEN - non-red error messages
toast({ title: "Failed", description: "Something went wrong" });
```

### WHY THIS MATTERS:
- Users CANNOT debug issues if errors disappear
- Users CANNOT report issues without error details
- Developers CANNOT fix issues without full context
- Support tickets are USELESS without copyable error details

### FILE LOCATION:
`src/lib/errorUtils.tsx` - Contains `showErrorToastWithCopy` and `getFullErrorText`
...
If you see `toast.error(` anywhere in the codebase, it is a bug that must be fixed.
