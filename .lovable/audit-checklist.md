...
- [fixed:CRITICAL — besties could self-approve their own comments via REST (INSERT with approval_status='approved' and PATCH to 'approved'), bypassing guardian review entirely. enforce_comment_approval_integrity trigger now pins approval_status on INSERT (forced to pending_approval when the guardian requires comment approval) and on UPDATE for non-guardian/non-admin actors. Verified: bestie INSERT with 'approved' → stored pending_approval; bestie PATCH → unchanged; guardian approve via /guardian-approvals UI → approved_by/approved_at set (e0e36239); guardian reject → rejected; bestie cannot un-reject; unrelated signed-in user sees only approved comments. Also fixed signup 500s: handle_new_user generated a random 3-emoji friend_code with no uniqueness retry — with 306 existing codes out of 8000 combos ~4% of all signups failed with 23505 unique_friend_code. Now retries (4-emoji fallback); 3 fresh signups all 200] guardian approval — comment
- [fixed:CRITICAL — guardian message approval was 100% broken in production: notify_on_sponsor_message_sent selected sponsorships.user_id, a column that does not exist, so every approval raised 42703 and rolled back; no sponsor could ever receive a bestie message. Fixed to use sponsor_id. Also dropped duplicate trigger on_sponsor_message_approved (second notification, ignored notification_preferences). Also besties could self-approve by POSTing status='approved' (INSERT policy had no status pin) and sponsors could rewrite the body/subject of a received message (UPDATE policy allowed all columns) — new enforce_sponsor_message_integrity trigger pins status/subject/message/media/approval fields for non-guardian/non-admin actors and forces new messages to pending_approval when the guardian requires approval. Verified: bestie INSERT with 'approved' → stored pending_approval; sponsor PATCH {"message":"HACKED"} → body unchanged; guardian REST approve → approved (19d55ec0) with exactly 1 sponsor notification + 1 sender confirmation; guardian UI /guardian-approvals → Messages tab → Approve As-Is → e7735656 approved, list shows "No pending messages to review" (screenshots). Test data and test sponsorship deleted; build OK] guardian approval — message
- [fixed] guardian approval — vendor asset — (1) vendor could publish a bestie's photo/voice publicly by inserting `approval_status:'approved'` (proved via REST: anon could read it); now pinned by `enforce_vendor_asset_approval_integrity` trigger. (2) Guardian UI dead end: `VendorLinkRequests` early-returned when there were no pending *link* requests, hiding the Asset Requests tab entirely — pending assets were unapprovable forever. (3) Vendor Links tab badge ignored pending assets. All three fixed & verified end-to-end (vendor insert → forced pending → guardian approve in UI → approved_by = guardian).

## Bestie Linking
- [fixed] bestie linking 3-emoji flow — two real vulns, both proved via REST and closed. (1) PII leak: `profiles_public` (readable by `anon`) exposed every member's `email` AND private `friend_code` — anyone could scrape all codes. Migration drops both columns from the view; re-query now returns 42703 for `friend_code` and no `email`. (2) Authorization bypass: `caregiver_bestie_links` INSERT policy had no role check and no friend-code proof — a plain `supporter` JWT could POST a guardian link to any bestie (reproduced 200 OK before fix). Fixed by admin-only INSERT policy + two SECURITY DEFINER RPCs: `find_bestie_by_friend_code` (exact-match only, non-enumerable) and `link_bestie_by_friend_code` (requires caregiver/admin role + correct code). Post-fix REST evidence: supporter link attempt → 42501 (RLS) / P0001 (role check); wrong code → P0001 "Friend code not found"; caregiver + correct code 🎭🌈🚀 → 200, link f0927568…. Frontend switched to the RPCs (`src/pages/GuardianLinks.tsx`, `src/components/vendor/VendorBestieLinkRequest.tsx`) and admin lookup moved to `profiles` table (`SponsorshipTransactionsManager.tsx`). UI verified end-to-end this turn as caregiver emailtest-link-caregiver-1787537386@example.com: Link Bestie → 🎭🌈🚀 + "parent" → toast "Link created — Successfully linked to Audit L", row `2946bb91-47dc-43ab-862f-ac8eb63a32f0` in `caregiver_bestie_links` (screenshot /tmp/browser/link-ui/ss/3_after.png). Typecheck: build OK. All audit fixtures deleted.
- [fixed] vendor bestie request — CRITICAL privilege escalation found in `vendors`: the "Vendors can update their own profile" RLS policy had no column restrictions, so any signed-up user could self-insert a pending vendor row then PATCH `status='approved'`, `commission_percentage=0`, `stripe_charges_enabled=true`. Proven: PATCH by vendor JWT returned `{"status":"approved","commission_percentage":0.00}` (user 16c9d744, vendor ae6577d5). This directly gates vendor↔bestie linking (INSERT policy only requires vendors.status='approved'). FIX: migration `enforce_vendor_privileged_fields` trigger (BEFORE INSERT OR UPDATE) pins status/approved_by/approved_at/rejection_reason/commission_percentage/all stripe_* /user_id for non-admin actors (admins + service_role/edge functions unaffected); INSERT forces pending + 10% commission. RE-TESTED: same PATCH now returns `status=pending, commission=10.00, stripe_charges_enabled=false` while `business_name` edit still succeeds ✅. Request flow verified after admin approval: POST with `status:"approved"` → 42501 ✅; POST pending → 201 (req 78a46984) ✅; vendor PATCH status→approved → 0 rows (no vendor UPDATE policy) ✅; pending-vendor POST → 42501 ✅. Guardian UI: /guardian-approvals → Vendor Links badge `1` → Approve → toast "Vendor link approved!", DB row status=approved, reviewed_by=6ac987c8 (guardian), reviewed_at set (screenshot /tmp/browser/vbr/ss/3_approved.png). NOTE (not fixed, low sev): vendor_bestie_requests INSERT does not require friend-code proof, so a vendor can request any bestie_id — spam vector only; guardian approval still required and no data is exposed pre-approval. All fixtures deleted (vendor, request, link, auth user = 0 rows). Build OK.
- [fixed:notify_on_sponsor_message_status_change null-sender crash] sponsor messaging — E2E 2026-08-24. Fixtures: sponsor e4f8494f (supporter, sponsorship b847891e $25/mo test mode), bestie ec84a7ce, guardian 6ac987c8 (require_message_approval=true). Evidence: (a) bestie POST sponsor_messages with status:"approved" → stored `pending_approval` (msg 55130063) ✅ item-79 trigger holds; (b) sponsor SELECT while pending → `[]` ✅; (c) guardian PATCH → approved, approved_by=6ac987c8 ✅; (d) sponsor SELECT → full message ✅; (e) sponsor PATCH is_read → true ✅; (f) sponsor PATCH message:"TAMPERED" → body unchanged ✅ (enforce_sponsor_message_integrity); (g) notification row type=message_approved to sent_by ✅. UI: /guardian-links as sponsor shows "Messages from Audit L", badge `1 message`, unread dot, header My Besties badge 1 (`/tmp/browser/sponsor-msg/gl.png`); expanding shows body and flips is_read→true (`expanded.png`). BUG FOUND+FIXED: guardian approval of any message with `sent_by IS NULL` raised 23502 (notifications.user_id NOT NULL) and aborted the whole approval transaction; trigger now returns early when sender is unknown (migration 2026-08-24).
- [fixed:new sponsor_message_reads table + inbox/badge read per sponsor] sponsor message read state per-sponsor — 2026-08-24. Root cause: `sponsor_messages.is_read` is one shared boolean per message, so the first sponsor to open cleared the unread dot/badge for every other sponsor of that bestie. Fix: new `public.sponsor_message_reads (message_id, sponsor_id, read_at)` with unique (message_id, sponsor_id), RLS scoped to `sponsor_id = auth.uid()` (+admin SELECT), grants to authenticated/service_role, and a backfill of already-read messages for all current active sponsors (8 rows carried over). `SponsorMessageInbox.tsx` now loads the caller's read marks and inserts one on open; `useSponsorUnreadCount.ts` counts messages minus the caller's read marks and subscribes to the new table. Evidence: message `da461a1e` + two active test-mode sponsorships (sponsors `e4f8494f`, `17ffe8cf`) on bestie `ec84a7ce` → sponsor A UI unread dot 1 → opened → dot 0, body rendered (`/tmp/browser/sponsor-msg/sponsorA_before.png`, `sponsorA_after.png`); sponsor B on same message still shows unread dot 1 (`sponsorB_before.png`); DB shows exactly one read row (sponsor A). RLS probes: B SELECT reads → `[]`, B insert with `sponsor_id`=A → 42501, anon SELECT → `[]`. tsgo clean; fixtures deleted.

## Notifications & Email
- [fixed:auth gate + HTML escaping + Mountain-time cron on send-digest-email] notification digest daily — E2E 2026-08-24. Fixture: user `d1c491a6` (profiles.email → `delivered@resend.dev` Resend sandbox), `notification_preferences.digest_frequency='daily'`, 2 unread notifications; `get_users_needing_digest('daily')` returned that one row. Baseline run (before fix): `{"processed":1,"successful":1,"failed":0}`, `digest_emails_log` row `d9d7107d` status=sent, Resend email_id `d4856833-aa3f-4f49-b9d3-58c5a67cf8e8`; second immediate call sent nothing (23h `last_digest_sent_at` guard holds ✅). FAIL 1 (auth): endpoint was fully public — POST with **no Authorization header at all** returned 200 and sent live email to real members; any anon could trigger/burn Resend quota and time the send. FAIL 2 (HTML injection): notification `title`/`message` were interpolated raw into the email HTML (fixture message `<img src=x onerror=...>` went out unescaped) — user-supplied comment/message text lands in admins' and members' inboxes. FAIL 3 (schedule): cron jobs 13/14 ran at `0 8 * * *` UTC = ~02:00 Mountain, contradicting the documented 8am send and the Mountain-time rule. Fixes: `DIGEST_CRON_SECRET` gate (cron secret / service-role / admin-owner JWT only) in `supabase/functions/send-digest-email/index.ts`, `escapeHtml()` applied to title, message, type label and logo URL, cron jobs 13+14 re-pointed with `X-Cron-Secret` and moved to `0 14 * * *` / `0 14 * * 1` (8am MDT). Re-test: no-auth POST → 401, anon-key JWT → 401, wrong cron secret → 401, OPTIONS → 200; with correct cron secret → `{"processed":1,"successful":1,"failed":0}`, log row status=sent, Resend email_id `97832828-88ab-401d-a6de-dae6437a08a4`; weekly with no eligible users → `{"processed":0}`. Note (not a defect, flagged for owner): all 225 rows in `notification_preferences` currently have `digest_frequency='never'` (the ProfileSettings default), so in production the digest reaches nobody until users opt in. Fixture user + notifications + log rows deleted.
- [pass] notification digest weekly — E2E 2026-08-24. Fixture: user `c0baa16b` (profiles.email → `delivered@resend.dev`), `digest_frequency='weekly'`, 3 unread notifications across 2 types. Evidence: (a) frequency isolation — `get_users_needing_digest('weekly')` = 1 row, `('daily')` = 0 rows ✅; (b) authorized run with `X-Cron-Secret` → `{"processed":1,"successful":1,"failed":0}`, `digest_emails_log` status=sent, notification_count=3, Resend email_id `adacedca-c769-4636-9b57-d28f9b7f27d1` ✅; (c) immediate second run → `{"processed":0}` (6d23h `last_digest_sent_at` guard holds) ✅; (d) daily run does not pick up the weekly user → `{"processed":0}` ✅; (e) no-auth POST → 401 (item-89 gate covers the weekly path too) ✅; (f) failure path — recipient switched to `@example.com`, run returned `successful:0 failed:1` and logged `status=failed` with the real Resend error text ("Invalid `to` field … domains like example.com"), i.e. no silent success ✅; (g) after that failure `last_digest_sent_at` stayed NULL and the user was still eligible, so a transient send error is retried next run instead of muting the user for a week ✅. Cron job 14 verified: schedule `0 14 * * 1` (Mon 8am MDT) with `X-Cron-Secret` header. Same caveat as daily: every production row in `notification_preferences` is `digest_frequency='never'`, so no real member receives a weekly digest until they opt in. Fixture deleted (profile row gone, log rows removed).
- [pass] newsletter send (send-newsletter) — E2E 2026-08-24. Fixtures: temp admin `a1d10000…00d1`, draft campaign `c1d10000…00c1` targeting `specific_emails: ["delivered@resend.dev"]`. Evidence: (a) CORS — OPTIONS 200 with `access-control-allow-origin: *` ✅; (b) auth — no header → 401 `UNAUTHORIZED_NO_AUTH_HEADER`; anon key → `Unauthorized`; authenticated **supporter** JWT → `Admin access required` and the probe campaign `c2` stayed `draft` (no status mutation before the admin check) ✅; (c) admin send → `{"success":true,"queuedCount":1,"totalSubscribers":1}`; `newsletter_email_queue` row status=`sent`, attempts=1, processed_at 02:50:43 (immediate processor trigger works, no cron wait) ✅; (d) delivery — `newsletter_emails_log` status=`sent`, Resend id `3531e6c0-7c4b-4eb3-9131-64be19754531` ✅; (e) campaign counters reconciled: status=`sent`, queued_count=1, processed_count=1, failed_count=0 ✅; (f) content pipeline — subject placeholders rendered (`Audit newsletter August 2026`), 2 rows in `newsletter_links`, personalized HTML contains both the `track-newsletter-click` rewrite and the `unsubscribe-newsletter` link ✅; (g) double-send guard — repeat POST → `Campaign already sent or sending`; bogus id → `Campaign not found` ✅. Minor (not blocking): all failures return HTTP 400 rather than 401/403, so callers can't distinguish auth failure from a bad campaign id. All fixtures deleted (campaigns, queue, log, links, temp admin user).
- [fixed:auth tiers + recipient proof + HTML escaping on send-automated-campaign] newsletter automated trigger — E2E 2026-08-24. **FAIL 1 (critical, open mail relay):** the function had *no* auth check at all and is not in `config.toml` (verify_jwt default off). Proof: `POST /functions/v1/send-automated-campaign` with **no Authorization header**, body `{"trigger_event":"newsletter_signup","recipient_email":"delivered@resend.dev"}` → `{"success":true,"template_used":"Welcome to Our Newsletter"}` and a real Resend send from `noreply@bestdayministries.org`. Any anonymous actor could blast any of the 5 active `auto_send` templates (`newsletter_signup`, `site_signup`, `subscription_created`, `vendor_approved`, `vendor_rejected`) to **arbitrary addresses**, burning the Resend quota and putting the sending domain's reputation at risk. **FAIL 2 (HTML injection):** `trigger_data` values were interpolated raw into the email body — caller-supplied `<img src=x onerror=…>` landed unescaped in recipients' inboxes. Fix in `supabase/functions/send-automated-campaign/index.ts`: (i) required-field validation → 400; (ii) 4-tier gate — service-role bearer and admin/owner JWT unrestricted, other authenticated callers may only mail **their own** address, anonymous callers limited to the three self-service triggers **and** the recipient must be provably self-registered in the last 15 min (`newsletter_subscribers.subscribed_at`/`created_at`, `profiles.created_at`, `sponsorships.sponsor_email`); (iii) `escapeHtml()` on all `trigger_data` values in the body plus `<>`-stripping in the subject, `$`-safe replacement. Re-test evidence: anon + unverified recipient → 403 "Recipient could not be verified"; anon + `vendor_approved` → 403 "Not authorized for this trigger"; empty body → 400; OPTIONS → 200; non-admin JWT → other recipient 403, **own** email passed the gate and reached Resend (500 only because `example.com` is rejected by Resend) ✅; stale subscriber (3 days old) → 403, then `subscribed_at` refreshed → anon send succeeded, `automated_campaign_sends` status=sent, Resend id `c4e62063-c433-4823-96d2-f97ebb465218` ✅ (legit public signup path preserved); escaping proved with temp template `audit_escape_probe` via admin JWT → log row has `&lt;img src=x onerror…` and no raw tag, Resend id `13e7f5d3-f5dd-400c-8d85-f75cbf5c41e6` ✅; 24h duplicate guard → `{"skipped":true}` ✅. All real callers still satisfied: `create-user`/`create-donation-checkout` (service role), `VendorManagement`/`AutomatedSendsLog` (admin JWT), `ProductForm`/`NewsletterPreferences`/`Auth`/`VendorAuth`/`SponsorshipSuccess` (self or fresh-signup path). Fixtures deleted (probe template, subscriber, send + log rows, temp admin role).
