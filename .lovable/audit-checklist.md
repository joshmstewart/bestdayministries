...
- [ ] sponsor message read state is per-message not per-sponsor — for a bestie with multiple sponsors, one sponsor opening a message sets is_read=true globally and clears every sponsor's unread badge (schema change needed: per-sponsor read table)
